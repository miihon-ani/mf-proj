<template>
  <div>
    ...
    <select v-model="selected">
      <option v-for="data in grow_type" :key="data.text" :value="data.value">
        {{ data.text }}
      </option>
    </select>
    <p>成長適正：すべてC、全週軽トレのみ（平均値）</p>
    <p v-if="life.length > 0">
      ...
      <br />
      <span>テスト：{{ culcStageWeek(4) }}</span>
      <br />
      <span>※成長タイプの端数処理は未実装です(取り急ぎ丸めただけです)</span>
    </p>
    <table v-if="life.length > 0">
      <tbody>
        <tr v-for="(value, index) in grow_stages[selected].value" :key="index">
          ...
          <td v-else></td>
          <td v-if="index < 10">
            {{ culcStageWeek(index) * culcTrainLight(index) }}
          </td>
...
export default {
  ...
  data() {
    return {
      ...
      trainning_light: [
        ...
        { stage: '第７段階', value: [3, 4, 5, 6] },
        { stage: '第８段階', value: [1, 2, 3, 4] }
      ],
      // 成長段階。後でローカルjsonにする
      grow_stages: [
  ...
  methods: {
    ...
    culcPeach(isGold) {
      ...
      const peak1 = this.culcStageWeek(4);
      if (peak1 > life) return 'ピーク：' + life;
      let return_msg = 'ピーク：' + peak1;
      life -= peak1;

      const peak2 = this.culcStageWeek(5);
    ...
    culcStage(stage) {
      ...
      if (stage < 0) return 'えらー';
      return Math.floor(
        this.grow_stages[this.selected].value[stage] * (this.life / 100)
      );
    },
    // 軽トレ用 現在は平均値のみ返している
